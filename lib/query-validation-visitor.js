...
import { GraphQLString, Kind, getNamedType, isInputObjectType, isListType, isNonNullType, BREAK, valueFromAST, typeFromAST, visit, getDirectiveValues } from "graphql";
import ValidationError from "./error.js";
import { getConstraintValidateFn, getScalarType } from "./type-utils.js";
import { constraintDirectiveTypeDefsObj } from "./type-defs.js";
...
function validateArrayTypeValue(context, valueTypeDef, typeDefWithDirective, value, currentField, argName, variableName, iFieldNameFull) {
    ...
    if (directiveArgumentMap) {
        ...
        if (directiveArgumentMap.minItems && (!value || value.length < directiveArgumentMap.minItems)) {
            context.reportError(new ValidationError(iFieldNameFull, errMessageBase + `must be at least ${directiveArgumentMap.minItems} in length`, [{ arg: 'minItems', value: directiveArgumentMap.minItems }]));
        }
        if (directiveArgumentMap.maxItems && value && value.length > directiveArgumentMap.maxItems) {
            context.reportError(new ValidationError(iFieldNameFull, errMessageBase + `must be no more than ${directiveArgumentMap.maxItems} in length`, [{ arg: 'maxItems', value: directiveArgumentMap.maxItems }]));
        }
...
export default (class QueryValidationVisitor {
    ...
    onOperationDefinitionEnter(operation) {
        ...
        operation.variableDefinitions ? [...operation.variableDefinitions] : [], this.options.variables ?? {}).coerced;
